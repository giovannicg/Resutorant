...
const getSortedVersions = async (name, declaration, options) => {
    ...
    if (protocolKnown) {
        ...
        tagsPromise = tagsPromise.then(() => (0, remote_git_tags_1.default)(`ssh://git@${host}/${path}`))
...
const greatestLevel = (level) => async (name, declaration, options = {}) => {
    const version = decodeURIComponent((0, parse_github_url_1.default)(declaration).branch)
        .replace(/^semver:/, '');
    const versions = await getSortedVersions(name, declaration, options);
    if (!versions)
        return null;
